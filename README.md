...
| `region` | 否 | 不指定 | ISO 3166-1 alpha-2 两位国家码（如 `cn`/`us`/`jp`），大小写不敏感；**仅 `bing` 生效**（映射为 Bing 的 `mkt` 市场参数），其余 Provider 静默忽略；`provider=auto` 时该参数不影响路由，只在最终选中 Bing 时生效 |
| `limit` | 否 | `10` | 1–20 |
| `page` | 否 | `1` | 1–10 |
...
`region` 是可选的 ISO 3166-1 alpha-2 两位国家码，语义和 `GET /v1/search` 的同名参数一致：仅 `bing` 生效，其余 Provider（包括 `auto` 路由到的非 Bing 结果）静默忽略。
