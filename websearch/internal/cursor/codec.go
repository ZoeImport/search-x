...
func (codec *Codec) Encode(state State) (string, error) {
	...
	sealed := codec.aead.Seal(nonce, nonce, plain, []byte("search-cursor-v1"))
	return "cur_v1_" + base64.RawURLEncoding.EncodeToString(sealed), nil
}
...
func (codec *Codec) Decode(token string) (State, error) {
	...
	sealed, err := base64.RawURLEncoding.DecodeString(strings.TrimPrefix(token, "cur_v1_"))
	if err != nil || len(sealed) <= codec.aead.NonceSize() {
		return State{}, ErrInvalid
	}
	nonce, body := sealed[:codec.aead.NonceSize()], sealed[codec.aead.NonceSize():]
	plain, err := codec.aead.Open(nil, nonce, body, []byte("search-cursor-v1"))
	if err != nil {
		return State{}, ErrInvalid
	}
	var state State
	if err := json.Unmarshal(plain, &state); err != nil || state.Version != 1 || state.ProviderPage < 1 || state.Limit < 1 || len(state.Providers) == 0 {
		return State{}, ErrInvalid
	}
