...
func TestRoundTripAndExpiry(t *testing.T) {
	...
	token, err := codec.Encode(State{QueryHash: QueryHash("go"), Provider: "bing", Providers: []string{"baidu", "bing"}, ProviderPage: 2, Limit: 10})
	if err != nil {
		t.Fatal(err)
	}
	state, err := codec.Decode(token)
	if err != nil || state.Provider != "bing" || state.ProviderPage != 2 {
